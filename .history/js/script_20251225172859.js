...
const loadMoreSvg = document.getElementById('load-more-svg'); // Добавляем SVG
...
function setupEventListeners() {
    ...
    loadMoreSvg.addEventListener('click', () => {
        ...
        if (visibleCoursesCount >= filteredCourses.length) {
            ...
            loadMoreSvg.classList.add('load-more-svg--rotate');
            
            // Убираем класс через время для повторной анимации
            setTimeout(() => {
                loadMoreSvg.classList.remove('load-more-svg--rotate');
            }, 300);
...
function updateLoadMoreButton() {
    ...
    if (filteredCoursesCount <= 9) {
        // Если курсов меньше или равно 9, скрываем кнопку и SVG
        loadMoreBtn.classList.add('load-more-btn--hidden');
        loadMoreSvg.style.display = 'none';
        loadMoreContainer.classList.remove('load-more-container--all-shown');
    } else if (allCoursesShown) {
